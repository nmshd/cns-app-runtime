...
export class MultiAccountController {
    ...
    public async init(): Promise<MultiAccountController> {
        ...
        this._db = await this.transport.createDatabase("accounts")
    ...
    public async getAccount(id: CoreId): Promise<LocalAccount> {
        ...
        if (!dbAccount) {
            throw TransportErrors.general.recordNotFound(LocalAccount, id.toString()).logWith(this._log)
        }
    ...
    public async selectAccount(id: CoreId, _masterPassword: string): Promise<[LocalAccount, AccountController]> {
        ...
        const accountController: AccountController = new AccountController(this.transport, db, this._config)
